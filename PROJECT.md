...
QuickResize uses the requested `0.26.8.2` style: `26.8.2` records August 2, 2026, while the leading `0` keeps the project in pre-1.0 status. The final component is the patch sequence for that date. Keep the exact version in `VERSION`, `APP_VERSION`, the changelog, release tag, and executable documentation synchronized.
...
- Push the release branch and create the GitHub release tag `v0.26.8.2`.
